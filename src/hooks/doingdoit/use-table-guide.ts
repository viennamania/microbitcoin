  ...
  */
  ...
  const fetchData = async (
    ...
    if (session?.user?.email.includes('@unove.space')) {
      ...
      const res = await fetch('/api/vienna/guide/getAll', {
        method: 'POST',
        headers: {
      ...
      const res = await fetch(`/api/vienna/guide/getAllByEmail?email=${session?.user?.email}&_limit=${countPerPage}&_page=${currentPage}&_sort=${sortConfig.key}&_order=${sortConfig.direction}&_q=${searchTerm}`);
  ...
  */
  ...
  function handleDelete(id: string | string[]) {
    ...
    const deleteData = async () => {
      ...
      await fetch(`/api/vienna/guide/deleteOne?id=${id}`);
